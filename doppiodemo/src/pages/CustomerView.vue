<template>
  <div class="h-full flex flex-col bg-gray-50 font-sans text-gray-900 selection:bg-orange-100 relative">
        ...
        <div v-else class="space-y-6">
          <div v-for="order in filteredOrders" :key="order.name"
              ...
              <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                <div class="flex justify-between items-center w-full sm:w-auto sm:gap-6">
                  ...
                  <span class="sm:hidden px-2 py-1 rounded text-[10px] font-bold uppercase border"
                    :class="getStatusClass(order)">{{ order.status }}</span>
                </div>
            ...
            <div class="p-4 sm:p-5 grid gap-4 relative">
              <div v-if="order.status === 'Cancelled' || order.docstatus === 2"
                class="absolute inset-0 bg-white/40 z-10 pointer-events-none"></div>
              <div v-for="item in order.items" :key="item.item_code"
                class="flex gap-3 sm:gap-4 items-center z-0 relative">
                <div class="w-12 h-12 border rounded bg-white p-1 flex-shrink-0 flex items-center justify-center"><img
                    v-if="item.image" :src="item.image" class="max-w-full max-h-full object-contain" />
            ...
            <div class="bg-gray-50/50 px-4 sm:px-5 py-3 border-t border-gray-100 flex justify-end gap-3">
              <button v-if="order.status !== 'Cancelled' && order.docstatus !== 2 && order.status !== 'Completed'"
                @click="handleCancelOrder(order.name)"
                class="text-[11px] font-bold text-red-500 bg-white hover:bg-red-50 border border-red-200 hover:border-red-500 px-4 sm:px-6 py-2 rounded shadow-sm uppercase flex items-center gap-1 transition-all active:scale-95"><span>✕</span>
                Cancel</button>
              <button v-if="order.status === 'Cancelled' || order.docstatus === 2"
                @click="handleDeleteOrder(order.name)"
                class="text-[11px] font-bold text-white bg-red-600 hover:bg-red-700 border border-red-600 px-4 sm:px-6 py-2 rounded shadow-sm uppercase flex items-center gap-1 transition-all active:scale-95"><span>🗑</span>
                Delete</button>
            </div>
          </div>
        </div>
      ...
      <div v-else-if="isNavigatingTree"
        ...
        <div class="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-gray-100 max-w-md w-full">
          ...
          <div
            ...
            <span :class="{ 'text-gray-900 font-bold': route.params.pincode }">Pin</span> › <span
              :class="{ 'text-gray-900 font-bold': route.params.society }">Soc</span> › <span
              :class="{ 'text-gray-900 font-bold': route.params.category }">Cat</span></div>
        </div>
      </div>
        ...
        <div v-else>
          <div
            ...
            <div>
              ...
              <div class="flex flex-wrap items-center gap-2 text-[10px] text-gray-500 mt-1"><span
                  class="bg-white border px-1.5 rounded">{{ route.params.society }}</span><span
                  class="text-gray-300">•</span><span class="bg-white border px-1.5 rounded">{{ route.params.category
                  }}</span></div>
            </div><span class="text-[10px] font-bold bg-gray-200 text-gray-700 px-2 py-1 rounded shadow-sm">{{
          ...
          <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 pb-20">
            <div v-for="item in filteredItems" :key="item.item_code"
              ...
              <div class="h-32 sm:h-40 p-4 flex items-center justify-center bg-white relative"><img v-if="item.image"
              ...
              <div class="p-3 flex flex-col flex-1 border-t border-gray-50 bg-gray-50/30">
                ...
                <div class="mt-auto flex items-center justify-between">
                  <div class="flex flex-col"><span class="text-base font-bold text-gray-900">₹{{ item.price
                      }}</span><span class="text-[10px] text-gray-400 line-through">₹{{ Math.round(item.price * 1.1)
                      }}</span></div><button @click="addToCart(item)"
      ...
      <div v-else-if="currentView === 'Cart'" class="max-w-3xl mx-auto animate-fade-in pb-10">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div class="px-5 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
            <h2 class="font-bold text-gray-800 text-lg">Shopping Cart</h2><span
              class="text-xs text-gray-500 font-medium bg-white px-2 py-1 rounded border">{{ cart.length }} items</span>
          </div>
          <div v-if="!cart.length" class="p-10 sm:p-16 text-center">
            <div class="text-6xl mb-4 grayscale opacity-20">🛒</div>
            <p class="text-gray-400 font-bold text-sm mb-6">Your cart is empty.</p><button
          ...
          <div v-else>
            <div class="divide-y divide-gray-100 max-h-[60vh] overflow-y-auto custom-scrollbar">
              <div v-for="(item, i) in cart" :key="i"
                ...
                <div class="flex items-center border border-gray-200 rounded-lg h-8 text-xs bg-white shadow-sm"><button
                    @click="updateQty(item, -1)" :disabled="item.qty <= 1"
                    class="px-2 hover:bg-gray-100 font-bold h-full disabled:opacity-30 text-gray-600 transition-colors">-</button><span
                    class="w-7 text-center font-bold bg-gray-50 h-full flex items-center justify-center border-x border-gray-200">{{
                    item.qty }}</span><button @click="updateQty(item, 1)"
                    class="px-2 hover:bg-gray-100 font-bold h-full text-gray-600 transition-colors">+</button></div>
                <div class="text-right min-w-[70px] sm:min-w-[80px]">
                  <p class="font-bold text-sm text-gray-900">₹{{ (item.price * item.qty).toFixed(2) }}</p><button
            ...
            <div
              ...
              </div><button @click="placeOrder" :disabled="isPlacingOrder"
                class="bg-gray-900 text-white px-6 sm:px-8 py-3 rounded-xl text-xs sm:text-sm font-bold uppercase hover:bg-black disabled:opacity-50 disabled:cursor-not-allowed shadow-lg flex items-center gap-2 transition-all active:scale-95"><span
                  v-if="isPlacingOrder"
                  class="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></span>{{
                    isPlacingOrder ? 'Processing...' : 'Place Order' }}</button>
            </div>
          </div>
        </div>
      </div>
...
const emit = defineEmits(['toggle-mobile', 'open-sidebar']) // ✅ Added open-sidebar emit

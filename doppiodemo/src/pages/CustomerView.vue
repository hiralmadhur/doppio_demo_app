<template>
  <div class="flex h-screen bg-gray-50 overflow-hidden relative font-sans text-gray-900 selection:bg-orange-100">
    ...
    <div
      class="fixed lg:static inset-y-0 left-0 z-50 bg-white shadow-2xl lg:shadow-none transform transition-all duration-300 ease-in-out h-full border-r border-gray-200"
      :class="[
        isMobileSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0',
        isDesktopCollapsed ? 'w-20' : 'w-72 xl:w-80'
      ]">
      <MarketSidebar :isCollapsed="isDesktopCollapsed && !isMobile" @select-action="handleSelection"
        @close-mobile="isMobileSidebarOpen = false" @toggle-collapse="isDesktopCollapsed = !isDesktopCollapsed" />
    </div>

    <div class="flex-1 flex flex-col min-w-0 h-full w-full relative transition-all duration-300">

      <header
        class="bg-white border-b border-gray-200 h-14 flex items-center justify-between px-4 sticky top-0 z-30 shadow-sm gap-2 sm:gap-4">
      ...
      <div class="flex-1 overflow-y-auto p-4 sm:p-6 custom-scrollbar scroll-smooth relative">
        ...
        <div v-if="currentView === 'Home'" class="h-full flex flex-col items-center pt-4 sm:pt-8 animate-fade-in">
          <div class="max-w-5xl w-full">
            <div
              class="bg-white rounded-2xl p-6 sm:p-10 shadow-sm border border-gray-100 mb-6 sm:mb-10 flex flex-col md:flex-row items-center justify-between text-center md:text-left relative overflow-hidden group">
              <div class="z-10 max-w-xl w-full">
                <h1 class="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-gray-900 mb-3 leading-tight">
                  ...
                  <span class="text-transparent bg-clip-text bg-gradient-to-r from-orange-600 to-yellow-500">{{
                    getUserName() }}</span>! 👋
                </h1>
              ...
              <div
                class="text-[100px] sm:text-[140px] md:text-[180px] opacity-5 md:opacity-10 absolute -right-6 -bottom-8 sm:-right-10 sm:-bottom-10 rotate-12 select-none pointer-events-none transition-transform group-hover:rotate-6 group-hover:scale-110 duration-700">
                🛒
              </div>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              <div @click="handleBrowseClick"
                class="bg-white p-5 sm:p-6 rounded-xl border border-gray-100 shadow-sm hover:shadow-lg hover:border-blue-100 transition-all cursor-pointer group active:scale-95">
                <div class="flex items-center gap-4">
                  <div
                    class="w-12 h-12 bg-blue-50 rounded-xl flex items-center justify-center text-2xl group-hover:scale-110 transition-transform duration-300">
                    🏪</div>
                  <div class="text-left">
              ...
              <div @click="router.push({ name: 'Orders' })"
                class="bg-white p-5 sm:p-6 rounded-xl border border-gray-100 shadow-sm hover:shadow-lg hover:border-orange-100 transition-all cursor-pointer group active:scale-95">
                <div class="flex items-center gap-4">
                  <div
                    class="w-12 h-12 bg-orange-50 rounded-xl flex items-center justify-center text-2xl group-hover:scale-110 transition-transform duration-300">
                    📦</div>
                  <div class="text-left">
              ...
              <div
                class="bg-white p-5 sm:p-6 rounded-xl border border-gray-100 shadow-sm hover:shadow-lg hover:border-green-100 transition-all cursor-default group">
                <div class="flex items-center gap-4">
                  <div
                    class="w-12 h-12 bg-green-50 rounded-xl flex items-center justify-center text-2xl group-hover:rotate-12 transition-transform duration-300">
                    🎧</div>
                  <div class="text-left">
        ...
        <div v-else-if="currentView === 'Orders'" class="max-w-4xl mx-auto space-y-5 animate-fade-in pb-10">
          <h2 class="text-xl sm:text-2xl font-bold text-gray-800 flex items-center gap-2"><span>📦</span> My Orders
          </h2>
          <div v-if="orders.loading" class="space-y-4">
            <div v-for="n in 3" :key="n" class="h-40 bg-white rounded-xl border border-gray-200 animate-pulse">
            </div>
          </div>
          <div v-else-if="!filteredOrders.length"
            ...
            <button @click="router.push({ name: 'Home' })"
              class="mt-4 text-blue-600 font-bold text-xs uppercase hover:underline">Start Shopping</button>
          </div>
          <div v-else class="space-y-6">
            <div v-for="order in filteredOrders" :key="order.name"
              class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow relative group">
              <div class="bg-gray-50 px-4 sm:px-5 py-3 border-b border-gray-100">
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                  <div class="flex justify-between items-center w-full sm:w-auto sm:gap-6">
                    ...
                    <span class="sm:hidden px-2 py-1 rounded text-[10px] font-bold uppercase border"
                      :class="getStatusClass(order)">
                      {{ getStatusLabel(order) }}
                    </span>
                  </div>
                  <div class="flex justify-between sm:justify-start gap-6 text-gray-600 w-full sm:w-auto">
                    <div><span class="block text-[10px] font-bold uppercase text-gray-400">Placed On</span><span
                        class="text-xs sm:text-sm font-bold text-gray-800">{{ order.transaction_date }}</span></div>
                    <div><span class="block text-[10px] font-bold uppercase text-gray-400">Total</span><span
                        class="text-xs sm:text-sm font-bold text-gray-800">₹{{ order.grand_total }}</span></div>
                  </div>
                  <span class="hidden sm:block px-2.5 py-1 rounded-full text-[10px] font-bold uppercase border"
                    :class="getStatusClass(order)">
                    {{ getStatusLabel(order) }}
                  </span>
                </div>
              ...
              <div class="p-4 sm:p-5 grid gap-4 relative">
                <div v-if="order.docstatus === 2" class="absolute inset-0 bg-white/40 z-10 pointer-events-none"></div>

                <div v-for="item in order.items" :key="item.item_code"
                  class="flex gap-3 sm:gap-4 items-center z-0 relative">
                  ...
                  <button @click="buyAgain(item, order.company)"
                    class="text-[10px] font-bold text-blue-600 bg-blue-50 px-3 py-1.5 rounded hover:bg-blue-100 transition-colors uppercase whitespace-nowrap border border-blue-100 active:scale-95">↻
                    Buy Again</button>
                </div>
              </div>
              <div class="bg-gray-50/50 px-4 sm:px-5 py-3 border-t border-gray-100 flex justify-end gap-3">
                
                <button v-if="order.docstatus === 0" @click="handleDeleteOrder(order.name)"
                  class="text-[11px] font-bold text-red-600 bg-white hover:bg-red-50 border border-red-200 px-4 py-2 rounded shadow-sm uppercase flex items-center gap-1 transition-all active:scale-95">
                  <span>🗑</span> Delete Order
                </button>

                <button v-if="order.docstatus === 1 && order.status !== 'Completed' && order.status !== 'Cancelled'"
                  @click="handleCancelOrder(order.name)"
                  class="text-[11px] font-bold text-orange-600 bg-white hover:bg-orange-50 border border-orange-200 px-4 py-2 rounded shadow-sm uppercase flex items-center gap-1 transition-all active:scale-95">
                  <span>✕</span> Cancel Order
                </button>

                <button v-if="order.docstatus === 2" @click="handleDeleteOrder(order.name)"
                  class="text-[11px] font-bold text-white bg-red-600 hover:bg-red-700 border border-red-600 px-4 py-2 rounded shadow-sm uppercase flex items-center gap-1 transition-all active:scale-95">
                  <span>🗑</span> Remove/Delete
                </button>
              </div>
            </div>
          </div>
        ...
        <div v-else-if="isNavigatingTree"
          class="h-full flex flex-col items-center justify-center text-center animate-fade-in px-4">
          <div class="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-gray-100 max-w-md w-full">
            ...
            <div
              class="flex flex-wrap items-center justify-center gap-2 text-xs text-gray-400 font-mono bg-gray-50 p-2 rounded border border-gray-100">
        ...
        <div v-else-if="currentView === 'SellerItems'" class="max-w-[1400px] mx-auto animate-fade-in">
          ...
          <div v-else>
            <div
              class="flex justify-between items-end mb-4 border-b border-gray-200 pb-3 sticky top-0 bg-gray-50 z-10 pt-2">
              <div>
                <h2 class="text-lg sm:text-xl font-bold text-gray-900 leading-tight">{{ route.params.sellerName }}
                </h2>
                <div class="flex flex-wrap items-center gap-2 text-[10px] text-gray-500 mt-1">
                  <span class="bg-white border px-1.5 rounded">{{ route.params.society }}</span><span
                    class="text-gray-300">•</span>
                  <span class="bg-white border px-1.5 rounded">{{ route.params.category }}</span>
                </div>
              </div>
              <span class="text-[10px] font-bold bg-gray-200 text-gray-700 px-2 py-1 rounded shadow-sm">{{
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 pb-20">
              <div v-for="item in filteredItems" :key="item.item_code"
                class="bg-white rounded-lg border border-gray-200 hover:border-orange-400 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex flex-col overflow-hidden group">
                ...
                <div class="p-3 flex flex-col flex-1 border-t border-gray-50 bg-gray-50/30">
                  <h3 class="font-semibold text-gray-800 text-[13px] leading-snug line-clamp-2 h-9 mb-2"
                    :title="item.item_name"><span v-html="highlightText(item.item_name)"></span></h3>
                  <div class="mt-auto flex items-center justify-between">
                    <div class="flex flex-col"><span class="text-base font-bold text-gray-900">₹{{ item.price
                        }}</span><span class="text-[10px] text-gray-400 line-through">₹{{ Math.round(item.price *
                        1.1)
                        }}</span></div>
                    <button @click="addToCart(item)"
                      class="bg-yellow-400 hover:bg-yellow-500 text-gray-900 px-3 py-1.5 rounded-md text-[11px] font-bold uppercase shadow-sm active:scale-95 transition-all">Add</button>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div v-else-if="currentView === 'Cart'" class="max-w-3xl mx-auto animate-fade-in pb-10">
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
              ...
              <span class="text-xs text-gray-500 font-medium bg-white px-2 py-1 rounded border">{{ cart.length }}
                items</span>
            </div>
            <div v-if="!cart.length" class="p-10 sm:p-16 text-center">
              ...
              <button @click="router.push({ name: 'Home' })"
                class="bg-gray-900 text-white px-6 py-2 rounded-lg text-xs font-bold uppercase hover:bg-black shadow-md transition-transform active:scale-95">Start
                Shopping</button>
            </div>
            <div v-else>
              <div class="divide-y divide-gray-100 max-h-[60vh] overflow-y-auto custom-scrollbar">
                <div v-for="(item, i) in cart" :key="i"
                  class="p-3 sm:p-4 flex gap-3 sm:gap-4 items-center hover:bg-gray-50 transition-colors">
                  <div
                    class="w-14 h-14 sm:w-16 sm:h-16 border rounded-lg bg-white p-1 flex-shrink-0 flex items-center justify-center">
                    <img v-if="item.image" :src="item.image" class="max-w-full max-h-full object-contain" />
                    <div v-else class="text-xl opacity-20">📦</div>
                  </div>
                  <div class="flex-1 min-w-0">
                    <p class="font-bold text-sm text-gray-900 truncate">{{ item.item_name }}</p>
                    <p class="text-xs text-gray-500 flex items-center gap-1"><span>🏪</span> {{ item.seller_name }}
                    </p>
                  ...
                  <div class="flex items-center border border-gray-200 rounded-lg h-8 text-xs bg-white shadow-sm">
                    <button @click="updateQty(item, -1)" :disabled="item.qty <= 1"
                      class="px-2 hover:bg-gray-100 font-bold h-full disabled:opacity-30 text-gray-600 transition-colors">-</button>
                    <span
                      class="w-7 text-center font-bold bg-gray-50 h-full flex items-center justify-center border-x border-gray-200">{{
                      item.qty }}</span>
                    <button @click="updateQty(item, 1)"
                      class="px-2 hover:bg-gray-100 font-bold h-full text-gray-600 transition-colors">+</button>
                  </div>
                  <div class="text-right min-w-[70px] sm:min-w-[80px]">
                    ...
                    <button @click="removeFromCart(i)"
                      class="text-[10px] text-red-500 font-bold hover:text-red-700 hover:underline mt-1">REMOVE</button>
                  </div>
                </div>
              </div>
              <div
                class="p-5 bg-gray-50 border-t border-gray-200 flex justify-between items-center shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                <div>
                ...
                <button @click="placeOrder" :disabled="isPlacingOrder"
                  class="bg-gray-900 text-white px-6 sm:px-8 py-3 rounded-xl text-xs sm:text-sm font-bold uppercase hover:bg-black disabled:opacity-50 disabled:cursor-not-allowed shadow-lg flex items-center gap-2 transition-all active:scale-95">
...
const handleCancelOrder = async (orderName) => {
  ...
  try {
    ...
    if (data.message && data.message.status === 'success') {
      ...
      await orders.fetch() // Wait for fetch to update UI so Delete button appears
    } else {
      toastRef.value?.add(data.message?.message || "Could not cancel", 'error')
    }
  } catch (e) {
    toastRef.value?.add("Network Error", 'error')
  }
}

const handleDeleteOrder = async (orderName) => {
  const msg = "Are you sure you want to permanently delete this order?";
  if (!await confirmRef.value.open('Delete Order?', msg)) return;
  ...
  try {
    ...
    if (data.message && data.message.status === 'success') {
      ...
      toastRef.value?.add(data.message?.message || "Could not delete", 'error')
    }
  } catch (e) {
    toastRef.value?.add("Network Error", 'error')
  }
...
const getStatusClass = (order) => {
  if (order.docstatus === 0) return 'bg-blue-50 text-blue-600 border-blue-200' // Pending
  if (order.docstatus === 2) return 'bg-gray-100 text-gray-500 border-gray-300' // Cancelled
  if (order.status === 'Completed') return 'bg-green-50 text-green-700 border-green-200'
  return 'bg-yellow-50 text-yellow-700 border-yellow-200' // Accepted/Submitted
}

const getStatusLabel = (order) => {
  if(order.docstatus === 2) return 'Cancelled'
  if(order.docstatus === 0) return 'Pending Acceptance'
  return order.status
}

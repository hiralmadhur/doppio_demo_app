<template>
    <div class="h-full flex flex-col bg-white border-r border-gray-200 text-gray-700 font-sans transition-all duration-300">
        ...
        <div class="h-14 flex items-center bg-gray-900 text-white shrink-0 shadow-sm px-3"
            ...
            <button @click="$emit('toggle-collapse')" class="hidden lg:block text-gray-400 hover:text-white p-1 rounded hover:bg-gray-800">
                {{ isCollapsed ? '»' : '«' }}
            </button>
        </div>

        <div class="flex-1 overflow-y-auto custom-scrollbar text-[13px]"> <div v-for="pin in treeData" :key="pin.pincode" class="border-b border-gray-50">
                ...
                <button @click="toggle(expandedPincodes, pin.pincode)" 
                    ...
                    <span v-if="!isCollapsed" class="text-[10px] text-gray-400" :class="isOpen(expandedPincodes, pin.pincode) ? 'rotate-180' : ''">▼</span>
                </button>
        ...
        <div class="p-3 border-t border-gray-200 bg-gray-50 shrink-0">
            <button @click="handleLogout" class="w-full flex items-center justify-center gap-2 text-red-600 font-bold text-xs hover:bg-red-50 py-2 rounded border border-transparent hover:border-red-100 transition-colors">
...
const fetchSidebarData = async () => {
    try {
        ...
        if (data.message.status === 'success') treeData.value = data.message.data
    } catch(e) {}
}
